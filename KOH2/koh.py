...
def rectangular_to_hexagonal(coords):
    x0 = coords[0]
    y0 = coords[1]
    x1 = 3/2*x0
    shift = (x0 % 2) * np.sqrt(3)/2
    y1 = y0 * np.sqrt(3) + shift
    return np.array([x1,y1])

class KohonenNetwork():
    def __init__(self, n, m, hex = False):
        self.n = n
        self.m = m
        self.weights = None
    ...
    def dist_sq(self, coords1, coords2):
        if self.hex:
            coords1 = rectangular_to_hexagonal(coords1)
            coords2 = rectangular_to_hexagonal(coords2)
        ...
        for _ in tqdm(range(iterations)):
            ...
            for k in range(self.n):
                for l in range(self.m):
                    ...
                    theta = self.neighborhood(self.dist_sq(coord_closest,  curr_coords))
