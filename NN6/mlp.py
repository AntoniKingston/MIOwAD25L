...
class MLP():
    ...
    def backpropagate(self, inputt, expected, metric, regularization=0):
        self.predict(inputt)
        ...
        for i in range(len(self.fed_values.array) - 1, 0, -1):
            ...
            if regularization == 0:
                regularization_weights = np.zeros_like(weight_grad.array[i-1])
                regularization_biases = np.zeros_like(bias_grad.array[i-1])
            elif regularization == 1:
                regularization_weights = np.sign(weight_grad.array[i-1])
                regularization_biases = np.sign(bias_grad.array[i-1])
            else:
                regularization_weights = 2 * self.weights.array[i-1]
                regularization_biases = 2 * self.biases.array[i-1]

            bias_grad.array[i-1] = neuron_fed_grad.array[i] + regularization_biases
            weight_grad.array[i-1] = np.outer(neuron_fed_grad.array[i], self.activation_values.array[i-1]) + regularization_weights
            last_layer = False
        return (weight_grad, bias_grad)

    def learn_batch(self, batchx, batchy, metric, regularization=0):
    ...
    def epoch(self, x, y, metric, lr=0.01, momentum_coef = 0, batch_size=None, regularization=0):
